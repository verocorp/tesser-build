...
// Package consumer05 reaches through to backend A's raw row — the coupling the
...
func Use(ctx context.Context, c orders.Client) (backend.OrderRowA, error) {
	resp, err := c.PlaceOrder(ctx, orders.PlaceOrderRequest{CustomerID: "c05", SKUs: []string{"x"}})
	if err != nil {
