...
// Package consumer13 reaches through to backend A's raw row — the coupling the
...
func Use(ctx context.Context, c orders.Client) (backend.OrderRowA, error) {
	resp, err := c.PlaceOrder(ctx, orders.PlaceOrderRequest{CustomerID: "c13", SKUs: []string{"x"}})
	if err != nil {
