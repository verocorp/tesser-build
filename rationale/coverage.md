...
| `voconstructor` | — dissolved (was `TB003` no-setattr-bypass) | the shell owns it: mutation through any door raises at runtime; the one sanctioned assignment site is `object.__setattr__` inside the VO's own `__init__` | — |
...
| — no Go analyzer (same gap) | `TB017` single-construction-door | a value object has ONE construction door — its own `__init__` (the 2026-07-20 (b)-uniform ruling, extended to collection VOs 2026-07-21): any classmethod/staticmethod returning its own type is a second door, name-agnostic (`from_spec`/`parse`/`new`/`require`/`of`), because two doors mean two invariant sets on one type. A factory returning another type, and any spec, are out of scope; entities/aggregates fall under `TB080`'s spec-only door | RULES.md |
...
| — no Go analyzer (same testing gap) | `TB071`+`TB073` test-module totality (supersede `TB032`) | testing norm (`testing.md`): every module-level function in a test module is a test, a declared `@ts.helper`, or a declared `@ts.fake`, and every class is a `Test`-prefixed test class holding only test methods or a declared `@ts.fake` (`TB071` — the totality half), and a helper takes only defaulted primitives, has no control flow, and builds a spec (`TB073` — the shape half); what does not fit declares itself or is a finding | RULES.md |
