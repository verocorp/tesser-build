...
| Jobs are an adapter kind (`adapters/jobs/`); reach is carried by the adapter kind package — a handler the context client, a job the application client / orchestrators / ports / gateways, a gateway or repository the ports; a component publishes exactly `client` and `jobs` | `map.md#adapters` | — | `python.md#orchestrators-actions-jobs` (verified impl `examples/durable-execution/`, exemplar `examples/minimal/`) | — | "writing a workflow on a durable-execution engine" |
