...
(candidate checkers for the `go/analysis` port — not scheduled). The ⚠️ row is
the one checker whose justification is ergonomic rather than a runtime-bug demo.
...
The two ❌ rows here are the same candidate checkers as the matrix above
(type-confusion, constructor-validation) — the `go/analysis` port closes them.
