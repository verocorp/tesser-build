...
| (construction) | `TB080` construction-doors (supersedes `TB013`) | a domain constructor takes exactly one `ts.Spec` (entity/aggregate); a value object constructs from primitives and value objects; a spec field is a primitive, a value object, or a child spec; a DTO field is a primitive or another DTO. An enum counts as a primitive (maintainer ruling 2026-08-23): a domain enum is a plain `enum.Enum` with no `ts.*` base, declared in a domain role module — `TB052` recognizes it (a `StrEnum`/`IntEnum` is a finding) and `TB051` keeps it members-only — and it is legal as a spec field and a value-object constructor parameter; client and port DTOs are untouched (a ports enum stays local to its ports module) | RULES.md |
