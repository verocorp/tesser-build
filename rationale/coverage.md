...
| — no Go analyzer (same testing gap) | `TB071`+`TB073` test-module totality (supersede `TB032`) | testing norm (`testing.md`): every module-level function in a test module is a test, a declared `@ts.helper`, or a declared `@ts.fake` (`TB071` — the totality half), and a helper takes only defaulted primitives, has no control flow, and builds a spec (`TB073` — the shape half); what does not fit declares itself or is a finding | RULES.md |
