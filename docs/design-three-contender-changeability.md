...
**Status:** design — Layer 1 BUILT (`rationale/inconsistent/` + its tests +
...
```
  ...
  high ┤  ███████ primitive        ██████ inconsistent
       │  bare primitives          inconsistent "VOs"
       │
       │
   ~0  ┤                                              ▏ valueobject
       │                                                consistent VOs
...
**The thesis predicts:** `primitive ≈ inconsistent ≫ valueobject ≈ 0`.

**Falsification condition (write it down now):** if `inconsistent` lands materially
below `primitive` — i.e. inconsistent VOs already capture most of the dividend — the thesis
...
|---|---|
| `primitive` | the concept is a bare `string`/`float64`/`map`, as `rationale/primitive/` |
| `inconsistent` | a **mixture of bare primitives and non-conforming value objects** — some concepts left primitive, some wrapped, the wrapped ones wrapped different ways (see modeling rules below). `rationale/inconsistent/` |
| `valueobject` | the concept is a conforming VO: private field, single validating constructor, value equality, no representation leak — `rationale/valueobject/` |

The inconsistent contender is a *mixture*, not "different kinds of VO." The realistic failure isn't
...
sites for each operation, each contender.
...
silent. The inconsistent contender's `Altitude` builders demonstrate exactly this — the compiler can't
...
## Modeling the inconsistent contender without rigging it (the grader=builder seam)

The inconsistent contender is a **fixture we construct**, so we choose how bad to make it. A
deliberately terrible strawman would rig the result and prove nothing. The
discipline: **every non-conforming shape in the inconsistent contender must be anchored to a real
non-conforming shape found in `certus` history** — not invented. Real anchors
already excavated:

| Inconsistent-contender non-conformance (modeled) | Real historical anchor | Silent-site leak it reopens | Checker that catches it |
|---|---|---|---|
...
This table is the spine of the deliverable: it simultaneously (a) defines the inconsistent contender,
...
1. **`rationale/inconsistent/`** — a third package mirroring the navigation domain
   (Feet/Meters, Temperature, etc.), implemented per the inconsistent-contender modeling rules
   above. Each file comments its real historical anchor.
2. **Extend `changeability_test.go` + `changeability_bench_test.go`** — run the
   change operations across all three contenders; assert the predicted ordering
...
4. **`case-study.md` (private magnitudes)** — add the `CreditType()`-string
   43-call-site partial-adoption example as the real-world magnitude for the
   inconsistent contender,
   mirroring how the rename/retype magnitudes already anchor the other two.
...
2. **Don't strawman the inconsistent contender** — every non-conforming shape cites a real historical
...
1. **Inconsistent-contender breadth:** **5 of the 6** shapes now have executable demos —
