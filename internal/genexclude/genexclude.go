// Package genexclude is the engine behind `tessercheck -gen-excludes`: it scans
...
// it can be written to a starter .tesser-build.yaml for a human to review.
//
...
// Render produces .tesser-build.yaml content from the classified entries. The date is
// passed in (not read from the clock) so the output is testable.
func Render(entries []Entry, date string) string {
	var b strings.Builder
	fmt.Fprintf(&b, "# Generated by `tessercheck -gen-excludes` on %s. Review and edit — each entry\n", date)
