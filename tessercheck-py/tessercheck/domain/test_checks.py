...
def test_placement_is_the_single_routing_decision() -> None:
    ...
    table = (
        ...
        ("shop.application.relays", True, "relays-init"),
        ("shop.application.relays", False, "relays-file"),
        ("shop.application.relays.orders", False, "relays"),
        ("shop.application.relays.sub.deep", False, "relays"),
        ("shop.application.relays.__main__", False, "relays"),
        ("shop.application.relays.test_orders", False, "test"),
        ("shop.application.relays.conftest", False, "conftest"),
        ("shop.application.snapshots", True, "snapshots-init"),
        ("shop.application.snapshots", False, "snapshots-file"),
        ("shop.application.snapshots.order", False, "snapshots"),
        ("shop.application.snapshots.sub.deep", False, "snapshots"),
        ("shop.application.snapshots.__main__", False, "snapshots"),
        ("shop.application.snapshots.test_order", False, "test"),
        ("shop.application.snapshots.conftest", False, "conftest"),
...
def test_a_role_must_be_a_package() -> None:
    ...
    assert not any(
        "flat.client" in f and "a role is a package, never a module" in f
        for f in findings
    ), findings
...
def test_every_declared_block_has_a_name_and_a_home() -> None:
    blocks = set(domain.TESSER_BASE_BLOCKS.values()) | set(domain.RELAY_MESSAGE_BLOCKS.values())
...
def test_service_dependencies_must_be_ports() -> None:
    ...
    assert any(
        ...
        and "parameter 'db' is not a ts.Port, a ts.Relay, or a ts.Store; a service depends only on ports, relays, and the stores that yield them" in f
        for f in findings
    )
...
def test_a_runtimes_module_holds_the_serdes_the_runtime_beside_it_binds() -> None:
    findings = tuple(
            ...
            (
                "shop/adapters/repositories/mixed.py",
                "shop.adapters.repositories.mixed",
                "import tesser.adapters as ts\n"
                "class SideGateway(ts.Gateway):\n"
                "    pass\n"
                "class ThingRepository(ts.Repository):\n"
                "    pass\n",
                False,
            ),
            (
                "shop/adapters/repositories/test_mixed.py",
                "shop.adapters.repositories.test_mixed",
                "def test_mixed() -> None:\n"
                "    assert True\n",
                False,
            ),
            (
                "shop/adapters/repositories/__init__.py",
                "shop.adapters.repositories",
                "from shop.adapters.repositories.mixed import ThingRepository as ThingRepository\n",
                True,
            ),
        ))).violations()
               )
...
def test_import_matrix_is_flagged() -> None:
    findings = tuple(
            ...
            (
                ...
                "two.domain.thing",
                "import tesser.domain as ts\n"
                "import two.client.client as client\n"
...
def test_only_a_handler_imports_its_own_client() -> None:
    findings = tuple(
            ...
            (
                ...
                "        return client.AskRequest(text=body).text\n",
                False,
            ),
            (
                "two/client/client.py",
    ...
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(
            ...
            (
                ...
                "    def __init__(self, text: str) -> None:\n"
                "        object.__setattr__(self, '_tag', tag.Tag(text))\n",
                False,
            ),
            (
                "mod/domain/test_label.py",
                "mod.domain.test_label",
                "import mod.domain.tag as tag\n"
                "def test_label() -> None:\n"
                "    assert tag.Tag('a') is not None\n",
                False,
            ),
            ("mod/domain/__init__.py", "mod.domain", "", True),
        ))).violations()
...
def test_a_context_tests_module_reaches_its_own_tests_package() -> None:
    findings = tuple(
            ...
            (
                ...
                "import shop.tests.conftest as conftest\n"
                "import two.tests.test_two as test_two\n"
                "def test_ok() -> None:\n    assert True\n",
                False,
            ),
...
def test_an_operation_is_named_for_a_verb_and_the_thing_it_acts_on() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_spec(sources=(
            (
            ...
            (
                ...
                "import typing\n"
                "import tesser.application as ts\n"
                "class SaveItemOutcome(enum.Enum):\n"
                "    STORED = 'stored'\n"
                ...
                "        self.text = text\n"
                "class SaveItemResponse(ts.Response):\n"
                "    def __init__(self, outcome: SaveItemOutcome) -> None:\n"
                "        self.outcome = outcome\n"
                "class LoadItemRequest(ts.Request):\n"
                "    def __init__(self, text: str) -> None:\n"
                "        self.text = text\n"
                "class LoadItemResponse(ts.Response):\n"
                "    def __init__(self, outcome: Verdict) -> None:\n"
                "        self.outcome = outcome\n"
                "class Record(ts.Response):\n"
                "    def __init__(self, text: str) -> None:\n"
                "        self.text = text\n"
    ...
    assert any(
        "shop.application.ports.sink.Sink.save names the operation save; an operation is a "
        "verb and the business thing it acts on, so its name has at least two segments" in f
        for f in findings
    ), findings
    assert any(
    ...
    assert any(
        "shop.application.ports.sink.Sink.keep_item takes a SaveItemRequest, not a "
        "KeepItemRequest; a request is named for the operation it carries, because a "
        "message named for anything else is a message two operations can share" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.ports.sink.Sink.keep_item answers a Record, not a "
        "KeepItemResponse; a response is named for the operation it carries, because a "
        "message named for anything else is a message two operations can share" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.ports.sink.Sink.load_item answers an outcome of Verdict, not "
        "LoadItemOutcome; an outcome is named for the operation that ends in it, because "
        "each member reads as a sentence after that act" in f
        for f in findings
    ), findings
...
def test_an_orchestrator_method_is_named_for_its_operation_and_never_run() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_kinds_spec(sources=(
            (
                "shop/application/orchestrators/named.py",
                "shop.application.orchestrators.named",
                "import tesser.application as ts\n"
    ...
    assert any(
        "shop.application.orchestrators.named.Named.run is run; an orchestrator method is "
        "named for the operation it carries, because run says only that it is invoked, and "
        "an orchestrator may carry more than one operation" in f
        for f in findings
    ), findings
    assert any(
    ...
    assert any(
        "shop.application.orchestrators.named.Named.price names the operation price; an "
        "operation is a verb and the business thing it acts on, so its name has at least "
        "two segments" in f
        for f in findings
    ), findings
    assert not any("Named.price_quote" in f and "TB085" in f for f in findings), findings


def test_a_client_names_its_operations_and_a_fake_of_it_is_not_asked_to() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_kinds_spec(sources=(
            (
                "shop/client/other.py",
                "shop.client.other",
                "import typing\n"
                "import tesser.context as ts\n"
                "class LookRequest(ts.Request):\n"
                "    def __init__(self, text: str) -> None:\n"
                "        self.text = text\n"
                "class LookResponse(ts.Response):\n"
                "    def __init__(self, text: str) -> None:\n"
                "        self.text = text\n"
            ...
            (
                "shop/adapters/handlers/test_cli.py",
                "shop.adapters.handlers.test_cli",
                "import tesser.testing as ts\n"
                "import shop.client.client as client\n"
                "@ts.fake\n"
                "class FakeClient(client.Client):\n"
                "    def ask(self, ask_question_request: client.AskQuestionRequest)"
                " -> client.AskQuestionResponse:\n"
    ...
    assert any(
        "shop.client.other.OtherClient.look names the operation look; an operation is a verb "
        "and the business thing it acts on, so its name has at least two segments" in f
        for f in findings
    ), findings
    assert not any("FakeClient.ask names the operation" in f for f in findings), findings
...
def test_a_conforming_ports_module_is_silent() -> None:
    findings = tuple(
            ...
            (
                ...
                "        self.id = id\n"
                "class SaveItemResponse(ts.Response):\n"
                "    def __init__(self, outcome: SaveItemOutcome, items: tuple[ItemView, ...]) -> None:\n"
                "        self.outcome = outcome\n"
                "        self.items = items\n"
...
def test_only_a_context_kernel_package_imports_a_root_kernel() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(('kernel/__init__.py', 'kernel', '', True), ('kernel/money.py', 'kernel.money', 'import tesser.domain as ts\nclass Money(ts.ValueObject):\n    _amount: int\n    def __init__(self, amount: int) -> None:\n        if amount < 0:\n            raise ValueError(f"negative: {amount}")\n        object.__setattr__(self, "_amount", amount)\n', False),
            ...
            (
                ...
                "shop.domain.kernel",
                "from kernel import Money as Money\n",
                True,
            ),
            ("kernel/__init__.py", "kernel", "from kernel.money import Money as Money\n", True)), imports=('money_kernel',))).violations()
    )
    ...
    assert any(
        "shop.domain.price imports kernel.money; a kernel is domain that two or more "
        "domain modules share, so only a context's domain kernel package imports a "
        "root kernel" in f
        for f in direct
    ), direct
    member = tuple(
...
def test_a_kernel_import_is_only_trusted_when_its_module_was_walked() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(('kernel/__init__.py', 'kernel', 'from kernel.money import Money as Money\n', True), ('kernel/money.py', 'kernel.money', 'import tesser.domain as ts\nclass Money(ts.ValueObject):\n    _amount: int\n    def __init__(self, amount: int) -> None:\n        if amount < 0:\n            raise ValueError(f"negative: {amount}")\n        object.__setattr__(self, "_amount", amount)\n', False),
            ...
            (
                ...
                "shop.domain.kernel",
                "from kernel import Money as Money\n"
                "from kernel.vendored.impure import Client as Client\n",
                True,
            ), ('shop/domain/price.py', 'shop.domain.price', 'import tesser.domain as ts\nimport shop.domain.kernel as kernel\nclass PriceSpec(ts.Spec):\n    def __init__(self, money: kernel.Money) -> None:\n        self.money = money\n', False),
            (
                "shop/domain/test_price.py",
                "shop.domain.test_price",
                "def test_price_exists() -> None:\n"
    ...
    assert any(
        "shop.domain.kernel imports kernel.vendored.impure; a role __init__ only "
        "re-exports a module of its own role, and a context kernel __init__ also "
        "re-exports from a root kernel" in f
        for f in findings
    ), findings
    assert not any(
        "shop.domain.kernel imports kernel;" in f for f in findings
    ), findings
...
def _kinds_spec(
    ...
    base: tuple[tuple[str, str, str | None, bool], ...] = (
        ...
        (
            ...
            "    def ask_question(self, ask_question_request: AskQuestionRequest) -> AskQuestionResponse: ...\n",
            False,
        ),
        (
            "shop/application/ports/__init__.py",
        ...
        (
            ...
            "from shop.adapters.runners.inline_quote_actions_relay import InlineFlowWorkflow as InlineFlowWorkflow\n",
            True,
        ),
        (
...
def test_an_application_client_module_speaks_one_ports_module() -> None:
    ...
    assert any(
        "shop.application.client.two_ports imports a second message package "
        "shop.application.ports.other; an application client module speaks the "
        "DTOs of exactly one ports or relays package" in f
        for f in findings
    )
    assert any(
        "shop.application.client.reaching imports shop.domain.thing; an application "
        "client module speaks the DTOs of exactly one ports or relays package" in f
        for f in findings
    )
    assert any(
        "shop.application.client.reaching imports no message package; an application "
        "client module speaks the DTOs of exactly one ports or relays package" in f
        for f in findings
    )
...
def test_an_application_client_module_holds_only_imports_and_one_protocol() -> None:
    ...
    assert any(
        ...
        "imports only tesser.application, one message package, and the pure stdlib" in f
        for f in findings
    )
    assert any(
...
def test_an_orchestrator_takes_action_ports_and_stores_only_them() -> None:
    ...
    assert any(
        ...
        "a ts.Port, a ts.Relay, or a ts.Store; an orchestrator depends only on ports, relays, and the stores that yield them" in f
        for f in findings
    )
    assert any(
...
def test_a_package_never_exports_a_class_of_its_own_name() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(
            (
                "mod/client/__init__.py",
                "mod.client",
                "from mod.client.client import Client as Client\n",
                True,
            ),
            (
                "mod/client/client.py",
            ...
            (
                "kernel/__init__.py",
                "kernel",
                "from kernel.identity import Kernel as Kernel\n",
                True,
            ),
            (
                "kernel/identity.py",
    ...
    assert any(
        "mod.client re-exports Client; a package never exports a class of its own "
        "name, because the name derived from the class rebinds the package's alias" in f
        for f in findings
    ), findings
    assert any(
        "kernel re-exports Kernel; a package never exports a class of its own name, "
        "because the name derived from the class rebinds the package's alias" in f
        for f in findings
    ), findings
...
def test_a_role_init_binds_no_module() -> None:
    ...
    assert any(
        "mod.domain __init__ binds a module; a role __init__ is the export list, and "
        "a module import exports nothing" in f
        for f in findings
    ), findings
...
def test_no_shell_module_and_no_role_outside_the_domain_imports_a_kernel() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(
            ...
            (
                ...
                "mod.domain.tag",
                "import tesser.domain as ts\n"
                "import mod.domain.kernel as kernel\n"
                "class Tag(ts.ValueObject):\n"
                "    def __init__(self, text: str) -> None:\n"
                "        object.__setattr__(self, '_id', kernel.Identity(text))\n",
                False,
            ),
            (
                "mod/domain/test_tag.py",
    ...
    assert not any("mod.domain.tag imports mod.domain.kernel" in f for f in findings), findings
    assert any(
        "mod.application.service imports mod.domain.kernel; a context's domain kernel "
        "is imported only by that context's domain modules, because outside the domain "
        "a kernel type is named through the domain __init__" in f
        for f in findings
    ), findings
    assert any(
        "app.wire imports kernel; neither an app nor a host imports a kernel, because "
        "a kernel is domain and the shell composes components, clients, adapters, and "
        "handlers" in f
        for f in findings
    ), findings
...
def test_a_two_hop_re_export_chain_resolves_to_the_defining_module() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(
            (
                "kernel/__init__.py",
                "kernel",
                "from kernel.identity import IdSpec as IdSpec\n",
                True,
            ),
            (
                "kernel/identity.py",
            ...
            (
                ...
                "class Rate(ts.ValueObject):\n"
                "    def __init__(self, id_spec: identity.IdSpec) -> None:\n"
                "        object.__setattr__(self, '_text', id_spec.text)\n",
                False,
            ),
            (
                "kernel/test_rates.py",
            ...
            (
                "mod/domain/kernel/__init__.py",
                "mod.domain.kernel",
                "from kernel import IdSpec as IdSpec\n",
                True,
            ),
            ("mod/domain/__init__.py", "mod.domain", "", True),
            (
                ...
                "import mod.domain.kernel as kernel\n"
                "class Tag(ts.ValueObject):\n"
                "    def __init__(self, id_spec: kernel.IdSpec) -> None:\n"
                "        object.__setattr__(self, '_text', id_spec.text)\n",
                False,
            ),
            (
    ...
    assert any(
        "mod.domain.tag.Tag takes kernel.identity.IdSpec, which kernel.rates.Rate "
    ...
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(
            ...
            (
                "mod/application/ports/__init__.py",
                "mod.application.ports",
                "from mod.application.ports.sink import SaveTextRequest as SaveTextRequest\n"
                "from mod.application.ports.sink import SaveTextResponse as SaveTextResponse\n"
            ...
            (
                ...
                "import typing\n"
                "import tesser.application as ts\n"
                "class SaveTextRequest(ts.Request):\n"
                "    def __init__(self, text: str) -> None:\n"
                "        self.text = text\n"
                "class SaveTextResponse(ts.Response):\n"
                "    def __init__(self, text: str) -> None:\n"
                "        self.text = text\n"
                "class Sink(ts.Port, typing.Protocol):\n"
                "    def save_text(self, save_text_request: SaveTextRequest) -> SaveTextResponse: ...\n",
                False,
            ),
            (
...
def test_a_relay_speaks_messages_that_carry_the_domain_and_snapshots_that_hold_nothing() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_kinds_spec(sources=(
            (
            ...
            (
                ...
                "    def __init__(self, text: str) -> None:\n"
                "        self.text = text\n"
                "class PlaceOrderRequestSnapshot(ts.Serde):\n"
                "    def serialize(self, place_order_request: PlaceOrderRequest) -> bytes:\n"
            ...
            (
                "shop/application/relays/test_orders.py",
    ...
    assert not any("shop.application.relays.orders" in f for f in findings), findings


def test_a_relay_dto_field_is_a_primitive_a_relay_dto_or_a_domain_object() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_kinds_spec(sources=(
            (
                "shop/application/relays/__init__.py",
                "shop.application.relays",
                "from shop.application.relays.bad import BadRequest as BadRequest\n",
                True,
            ),
            (
                "shop/application/relays/bad.py",
                "shop.application.relays.bad",
                "import os\n"
                "import tesser.application as ts\n"
                "class BadRequest(ts.Request):\n"
                "    def __init__(self, flag: bool, table: dict[str, str]) -> None:\n"
                "        self.flag = flag\n"
                "        self.table = table\n"
                "class BadSnapshot(ts.Serde):\n"
                "    LIMIT = 3\n"
                "    def __init__(self, held: str) -> None:\n"
                "        self._held = held\n"
                "    def serialize(self, bad_request: BadRequest) -> bytes:\n"
                "        return b''\n",
                False,
            ),
            (
                "shop/application/relays/test_bad.py",
                "shop.application.relays.test_bad",
                "import shop.application.relays as relays\n"
                "def test_bad_exists() -> None:\n"
                "    assert relays.BadRequest is not None\n",
                False,
            ),
        ))).violations()
    )
    assert any(
        "shop.application.relays.bad.BadRequest.__init__ field 'flag' is a bool; "
        "a relay DTO field is never a bare bool — model the outcome as an enum" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.relays.bad.BadRequest.__init__ parameter 'table' is not "
        "allowed; a relay DTO field is a primitive, another relay DTO, or a domain "
        "object, because both ends of a relay are this context" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.relays.bad.BadSnapshot declares no deserialize; a snapshot "
        "declares serialize and deserialize and nothing else, because a snapshot is "
        "the one place a message becomes bytes" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.relays.bad.BadSnapshot.__init__ is a method; a snapshot "
        "declares serialize and deserialize and nothing else, because a snapshot is "
        "the one place a message becomes bytes" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.relays.bad.BadSnapshot carries a class-level statement; a "
        "snapshot holds nothing, because a relay writes the same bytes on every replay" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.relays.bad.BadSnapshot stores '_held'; a snapshot holds "
        "nothing, because a relay writes the same bytes on every replay" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.relays.bad imports os; a relays or snapshots module imports "
        "the application stdlib and json, because a snapshot is where a message "
        "becomes bytes" in f
        for f in findings
    ), findings
...
def test_relays_and_snapshots_are_packages_never_modules() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_spec(sources=(
            ...
            (
                "shop/application/snapshots.py",
                "shop.application.snapshots",
                "import tesser.application as ts\n"
                "class ThingSnapshot(ts.Serde):\n"
    ...
    assert any(
        "shop.application.relays is a relays module; relays is a package, never a module" in f
        for f in findings
    ), findings
    assert any(
        "shop.application.snapshots is a snapshots module; snapshots is a package, "
        "never a module" in f
        for f in findings
    ), findings


def test_a_runner_reaches_its_relays_and_a_runtime_what_it_registers() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_kinds_spec(sources=(
            (
            ...
            (
                "shop/adapters/runners/engine.py",
                "shop.adapters.runners.engine",
                "import tesser.adapters as ts\n"
                "import shop.application.relays as relays\n"
                "import shop.application.service as service\n"
            ...
            (
                "shop/adapters/runners/test_engine.py",
                "shop.adapters.runners.test_engine",
                "import shop.adapters.runners as runners\n"
                "def test_engine_exists() -> None:\n"
                "    assert runners.EngineRunner is not None\n",
                False,
            ),
        ))).violations()
    )
...
def test_a_far_side_is_read_through_a_local_alias_of_a_workflow_or_a_client() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_kinds_spec(sources=(
            (
...
def test_an_await_method_reads_the_promise_named_for_its_operation() -> None:
    findings = tuple(
        ...
        for v in domain.Codebase(_kinds_spec(sources=(
            ...
            (
                "shop/adapters/runtimes/desk.py",
                "shop.adapters.runtimes.desk",
                "import tesser.adapters as ts\n"
                "import engine\n"
                "class DeskRuntime(ts.Runtime):\n"
                "    def __init__(self) -> None:\n"
                "        self.confirm_order_workflow = engine.Workflow('ConfirmOrder')\n"
                "        @self.confirm_order_workflow.main()\n"
                "        async def confirm_order(engine_context: engine.Context) -> None:\n"
                "            await engine_context.promise('pack_order').peek()\n"
    ...
    assert any(
        f"{where}.EngineWatchSignalRelay.await_pack_order reaches the promise quote_price, which no "
        "runtime in shop registers; a runner reaches only a service, handler, or promise a runtime "
        "of its context registers, because the name is all that holds the two ends together" in f
        for f in findings
    ), findings
...
def test_a_runner_is_its_engine_and_its_relay_and_mirrors_that_relay() -> None:
    ...
    assert any(
        f"{where}.EngineOrderRunner ends in no relay's or workflow's name; a runner is its engine's "
        "word followed by the name of the relay or workflow it implements, because its name is how "
        "that protocol is found" in f
        for f in findings
    ), findings
    assert any(
    ...
    assert any(
        f"{where}.EngineConfirmOrderRelay.run_confirm_order reaches ConfirmOrder.cancel_order, which "
        "no runtime in shop registers" in f
        for f in findings
    ), findings
    assert any(
        f"{where}.ElsewhereConfirmOrderRelay.start_confirm_order reaches Billing.confirm_order, which "
        "no runtime in shop registers" in f
        for f in findings
    ), findings
...
def test_a_workflow_beside_an_application_client_yields_that_client() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_kinds_spec(sources=(
            (
...
@ts.helper
def _engine_kinds_spec(old: str = "", new: str = "", old_also: str = "", new_also: str = "") -> domain.CodebaseSpec:
    replaced = {
        "shop/application/relays/__init__.py": (
            "shop.application.relays",
            "from shop.application.relays.flow_relay import IssueQuoteRequest as IssueQuoteRequest\n"
    ...
    files: dict[str, tuple[str, str, bool]] = {
        ...
        "shop/adapters/activities/engine_activities.py": (
            ...
            "    def __init__(\n"
            "        self, quote_actions_service: engine.Service, shop_application_client: client.ShopApplicationClient\n"
            "    ) -> None:\n"
        ...
        "shop/adapters/workflows/engine_issue_quote.py": (
            ...
            "    def __init__(\n"
            "        self, engine_workflow_context: engine.WorkflowContext, engine_quote_price: activities.EngineQuotePrice\n"
            "    ) -> None:\n"
            ...
            "    def __init__(self, engine_workflow_context: engine.WorkflowContext) -> None:\n"
            "        self._engine_workflow_context = engine_workflow_context\n"
            ...
            "    def __init__(self, flow_workflow: engine.Workflow, engine_quote_price: activities.EngineQuotePrice) -> None:\n"
            "        @flow_workflow.main()\n"
            "        async def issue_quote(\n"
            "            engine_workflow_context: engine.WorkflowContext, issue_quote_request: relays.IssueQuoteRequest\n"
            "        ) -> relays.IssueQuoteResponse:\n"
        ...
        "shop/adapters/dispatchers/engine_http_flow_relay.py": (
            ...
            "    def __init__(self, flow_workflow: engine.Workflow) -> None:\n"
            "        @flow_workflow.handler()\n"
            "        async def quote_accepted(\n"
            "            engine_workflow_shared_context: engine.WorkflowSharedContext,\n"
        ...
        "shop/component/engine.py": (
            ...
            "    def __init__(self, engine_client: engine.Client) -> None:\n"
            "        self.quote_actions_service: engine.Service = engine.Service('QuoteActions')\n"
            "        self.flow_workflow: engine.Workflow = engine.Workflow('Flow')\n"
...
def test_an_engine_container_is_named_for_the_far_side_of_what_registers_into_it() -> None:
    findings = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_engine_kinds_spec(
            old="engine.Service = engine.Service('QuoteActions')",
            new="engine.Service = engine.Service('Quotes')",
            old_also="engine.Workflow('Flow')",
            new_also="engine.Workflow('Flows')",
        )).violations()
    )
    assert any(
    ...
    loose = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_engine_kinds_spec(
            old="        self.flow_workflow: engine.Workflow = engine.Workflow('Flow')\n",
            new=(
                "        self.flow_workflow: engine.Workflow = engine.Workflow('Flow')\n"
                "        self.loose_workflow: engine.Workflow = engine.Workflow('Loose')\n"
            ),
    ...
    unnamed = tuple(
        f"{v.path()}:{int(v.line())}: {v.code()} {v.text()}"
        for v in domain.Codebase(_engine_kinds_spec(
            old="engine.Workflow('Flow')",
            new='engine.Workflow(FLOW)',
        )).violations()
    )
    assert any(
