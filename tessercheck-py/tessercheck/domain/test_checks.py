...
def test_a_violation_spec_carries_all_four_fields() -> None:
    ...
    with pytest.raises(RuntimeError, match="exactly the four"):
        checks.Rulebook(checks.RulebookSpec(checks_text))
...
def test_a_violation_spec_may_name_its_fields() -> None:
    ...
    rendered = str(checks.Rulebook(checks.RulebookSpec(checks_text)))
...
def test_an_assert_literal_containing_the_clause_is_the_fixture() -> None:
    ...
    rendered = str(checks.Rulebook(checks.RulebookSpec(checks_text, modules)))
...
def test_contracts_pair_each_section_id_with_its_name() -> None:
    ...
    rendered = str(checks.Rulebook(checks.RulebookSpec(checks_text, contracts_text=text)))
...
def test_an_applies_to_row_nothing_produces_is_a_loud_failure() -> None:
    ...
    with pytest.raises(RuntimeError, match="APPLIES_TO rows nothing produces"):
        checks.Rulebook(checks.RulebookSpec(checks_text, total=True))
