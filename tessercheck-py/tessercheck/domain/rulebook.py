...
HOLE_NAMES: typing.Final[dict[str, str]] = {
    "reached_name": "⟨service.handler or promise⟩",
    "len(workflows)": "⟨count⟩",
...
APPLIES_TO: typing.Final[dict[str, str]] = {
    ...
    "ClassDecl.handler_client_violations": "public handler method",
    ...
    "Module.container_violations": "component class",
...
class Rulebook(ts.ValueObject):
    ...
    def __init__(self, spec: RulebookSpec) -> None:
        ...
        lines += [
            ...
            "  is homeless.",
            "- the runner and runtime kinds (`ts.Runner` in `adapters/runners/`,",
            "  `ts.Runtime` in `adapters/runtimes/`) carry placement and import rules",
            "  only — no signature or body rules yet (deliberate: reach is what the",
            "  split exists for; what a runner or a runtime may hold is a later",
            "  wave).",
