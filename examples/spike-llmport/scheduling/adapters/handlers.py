...
class LlmToolHandler(ts.Handler):
    ...
    def __init__(self, client: SchedulingClient, booking_id: str) -> None:
        self._client = client
        self._booking_id = booking_id

    def begin(self) -> BookingStateResponse:
        return self._client.begin(BeginBookingRequest(booking_id=self._booking_id))

    def status(self) -> BookingStateResponse:
        return self._client.status(StatusRequest(booking_id=self._booking_id))

    def tools(self, state: BookingStateResponse) -> tuple[dict[str, object], ...]:
    ...
    def _confirm(self) -> BookingStateResponse:
        try:
            return self._client.confirm(
                ConfirmBookingRequest(booking_id=self._booking_id)
            )
        except ValueError as err:
            state = self._client.reoffer(ReofferRequest(booking_id=self._booking_id))
            offered = ", ".join(state.offered_slots)
            raise ValueError(f"{err}; now available: {offered}") from err

    def _schema(self, tool: str, state: BookingStateResponse) -> dict[str, object]:
        if tool == PROVIDE_NAME:
            return {
                "name": tool,
