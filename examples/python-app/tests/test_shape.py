...
def test_public_interface_is_client_plus_dtos_in_the_client_module() -> None:
    ...
    for ctx in discovery.discovered_contexts():
        init_source = (support.ROOT / ctx / "__init__.py").read_text()
...
class FakeReportsClientFailing(reports_client.Client):
    def links_by_verdict(self, req: reports_client2.LinksByVerdictRequest) -> reports_client2.LinksByVerdictResponse:
        raise errors.InfraError("the campaign store is unreachable")


def test_reports_handler_translates_client_dtos_to_component() -> None:
    resp = reports_http.Handler(FakeReportsClientStub()).links_by_verdict(protocol_http.HttpRequest("GET", "/", {}, {}, {}, b""))
...
def test_reports_handler_maps_a_failure_to_a_problem_document() -> None:
    try:
        resp = reports_http.Handler(FakeReportsClientFailing()).links_by_verdict(protocol_http.HttpRequest("GET", "/", {}, {}, {}, b""))
        ...
        resp = protocol_http.HttpResponse.problem(503, "unavailable", "a dependency is unavailable; please retry")
    except Exception:
