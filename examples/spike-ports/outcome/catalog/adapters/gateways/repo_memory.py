...
class MemoryItemRepository(ts.Repository):
    ...
    def find(self, request: item_repository.FindItemRequest) -> item_repository.FindItemResponse:
        ...
        if row is None:
            return item_repository.FindItemResponse(outcome=item_repository.MISSING, items=())
        return item_repository.FindItemResponse(outcome=item_repository.FOUND, items=(row,))
