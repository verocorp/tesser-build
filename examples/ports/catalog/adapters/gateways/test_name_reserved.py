...
def test_a_reserved_name_is_refused_with_a_reason() -> None:
    ...
    check_name_response = reserved_name_policy.check_name(ports.CheckNameRequest(name="admin"))
    assert (check_name_response.outcome, check_name_response.reason) == (
        ports.CheckNameOutcome.RESERVED,
        "name is reserved",
    )
...
def test_a_free_name_is_allowed_without_a_reason() -> None:
    reserved_name_policy = gateways.ReservedNamePolicy(reserved=("admin",))
    check_name_response = reserved_name_policy.check_name(ports.CheckNameRequest(name="Anvil"))
    assert (check_name_response.outcome, check_name_response.reason) == (
        ports.CheckNameOutcome.ALLOWED,
        "",
    )
...
def test_every_entry_in_the_reserved_list_is_refused() -> None:
    ...
    first = reserved_name_policy.check_name(ports.CheckNameRequest(name="admin"))
    second = reserved_name_policy.check_name(ports.CheckNameRequest(name="root"))
...
def test_a_reserved_name_is_matched_by_case() -> None:
    reserved_name_policy = gateways.ReservedNamePolicy(reserved=("admin",))
    check_name_response = reserved_name_policy.check_name(ports.CheckNameRequest(name="Admin"))
    assert check_name_response.outcome is ports.CheckNameOutcome.ALLOWED


def test_a_name_that_merely_contains_a_reserved_entry_is_allowed() -> None:
    reserved_name_policy = gateways.ReservedNamePolicy(reserved=("admin",))
    check_name_response = reserved_name_policy.check_name(ports.CheckNameRequest(name="administrator"))
    assert check_name_response.outcome is ports.CheckNameOutcome.ALLOWED
...
def test_an_empty_name_is_allowed_when_it_is_not_reserved() -> None:
    reserved_name_policy = gateways.ReservedNamePolicy(reserved=("admin",))
    check_name_response = reserved_name_policy.check_name(ports.CheckNameRequest(name=""))
    assert check_name_response.outcome is ports.CheckNameOutcome.ALLOWED
