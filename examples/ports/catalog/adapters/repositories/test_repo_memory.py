...
def test_a_saved_item_is_found_by_its_id() -> None:
    ...
    memory_item_repository.save_item(ports.SaveItemRequest(id="a1", name="Anvil"))
    find_item_response = memory_item_repository.find_item(ports.FindItemRequest(id="a1"))
    match find_item_response.outcome:
        case ports.FindItemOutcome.FOUND:
            assert tuple(
                (view.id, view.name) for view in find_item_response.items
            ) == (("a1", "Anvil"),)
        case ports.FindItemOutcome.ARCHIVED | ports.FindItemOutcome.NOT_FOUND:
...
def test_an_id_that_was_never_saved_is_missing_and_carries_nothing() -> None:
    memory_item_repository = repositories.MemoryItemRepository()
    find_item_response = memory_item_repository.find_item(ports.FindItemRequest(id="ghost"))
    assert (find_item_response.outcome, find_item_response.items) == (
        ports.FindItemOutcome.NOT_FOUND,
        (),
    )
...
def test_saving_an_id_again_replaces_the_stored_name() -> None:
    memory_item_repository = repositories.MemoryItemRepository()
    memory_item_repository.save_item(ports.SaveItemRequest(id="a1", name="Anvil"))
    memory_item_repository.save_item(ports.SaveItemRequest(id="a1", name="Anvil Mk II"))
    find_item_response = memory_item_repository.find_item(ports.FindItemRequest(id="a1"))
    list_items_response = memory_item_repository.list_items(ports.ListItemsRequest())
...
def test_all_answers_every_saved_item_in_the_order_they_were_saved() -> None:
    memory_item_repository = repositories.MemoryItemRepository()
    memory_item_repository.save_item(ports.SaveItemRequest(id="a1", name="Anvil"))
    memory_item_repository.save_item(ports.SaveItemRequest(id="b2", name="Bellows"))
    list_items_response = memory_item_repository.list_items(ports.ListItemsRequest())
...
def test_two_repositories_do_not_share_their_rows() -> None:
    ...
    first.save_item(ports.SaveItemRequest(id="a1", name="Anvil"))
    find_item_response = second.find_item(ports.FindItemRequest(id="a1"))
    assert find_item_response.outcome is ports.FindItemOutcome.NOT_FOUND
