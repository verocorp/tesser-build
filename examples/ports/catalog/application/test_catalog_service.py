...
class FakeNamePolicy(ports.NamePolicy):
    ...
    def __init__(self, outcome: ports.CheckNameOutcome, reason: str) -> None:
        self.outcome = outcome
        self.reason = reason
        self.asked: list[str] = []
    ...
    def check_name(self, check_name_request: ports.CheckNameRequest) -> ports.CheckNameResponse:
        ...
        return ports.CheckNameResponse(outcome=self.outcome, reason=self.reason)


def test_adding_an_item_answers_with_what_was_stored() -> None:
    catalog_service = application.CatalogService(
        FakeItemRepository(outcome=ports.FindItemOutcome.FOUND),
        FakeNamePolicy(outcome=ports.CheckNameOutcome.ALLOWED, reason=""),
    )
    add_item_response = catalog_service.add_item(client.AddItemRequest(id="a1", name="Anvil"))
...
def test_adding_an_item_asks_the_policy_about_the_requested_name() -> None:
    fake_name_policy = FakeNamePolicy(outcome=ports.CheckNameOutcome.ALLOWED, reason="")
    catalog_service = application.CatalogService(
        FakeItemRepository(outcome=ports.FindItemOutcome.FOUND), fake_name_policy
    )
    catalog_service.add_item(client.AddItemRequest(id="a1", name="Anvil"))
...
def test_a_refused_name_answers_blank_with_the_reason_the_policy_gave() -> None:
    catalog_service = application.CatalogService(
        FakeItemRepository(outcome=ports.FindItemOutcome.FOUND),
        FakeNamePolicy(outcome=ports.CheckNameOutcome.RESERVED, reason="name is reserved"),
    )
    add_item_response = catalog_service.add_item(client.AddItemRequest(id="c3", name="admin"))
...
def test_an_item_with_no_name_is_refused_before_any_port_is_touched() -> None:
    fake_item_repository = FakeItemRepository(outcome=ports.FindItemOutcome.FOUND)
    fake_name_policy = FakeNamePolicy(outcome=ports.CheckNameOutcome.ALLOWED, reason="")
    catalog_service = application.CatalogService(fake_item_repository, fake_name_policy)
    with pytest.raises(ValueError, match="name must be non-empty"):
        catalog_service.add_item(client.AddItemRequest(id="a1", name=""))
...
def test_getting_an_added_item_answers_it() -> None:
    ...
    catalog_service.add_item(client.AddItemRequest(id="a1", name="Anvil"))
    get_item_response = catalog_service.get_item(client.GetItemRequest(id="a1"))
...
def test_getting_an_id_that_was_never_added_answers_nothing() -> None:
    ...
    get_item_response = catalog_service.get_item(client.GetItemRequest(id="ghost"))
...
def test_listing_answers_every_added_item() -> None:
    ...
    catalog_service.add_item(client.AddItemRequest(id="a1", name="Anvil"))
    catalog_service.add_item(client.AddItemRequest(id="b2", name="Bellows"))
    list_items_response = catalog_service.list_items(client.ListItemsRequest())
...
def test_an_archived_lookup_carries_nothing_even_though_it_carries_a_row() -> None:
    find_item_response = ports.FindItemResponse(
        outcome=ports.FindItemOutcome.ARCHIVED,
        items=(ports.Item(id="a1", name="Anvil"),),
    )
    get_item_response = application.MapToGetItemResponse(
        find_item_response=find_item_response
...
def test_a_missing_lookup_carries_nothing() -> None:
    find_item_response = ports.FindItemResponse(outcome=ports.FindItemOutcome.NOT_FOUND, items=())
    get_item_response = application.MapToGetItemResponse(
        find_item_response=find_item_response
    )
    assert get_item_response.items == ()
...
def test_an_allowed_name_carries_the_item_and_the_policys_empty_reason() -> None:
    ...
    check_name_response = ports.CheckNameResponse(outcome=ports.CheckNameOutcome.ALLOWED, reason="")
