...
class TestRestateHost:
    ...
    def test_the_served_endpoint_discovers_the_actions_service_and_the_orchestrator_workflow(self) -> None:
        ...
        assert {s["name"]: sorted(h["name"] for h in s["handlers"]) for s in services} == {
            "WidgetActions": ["keep_widget"],
            "WidgetOrchestrator": ["approve_widget", "register_widget"],
        }
