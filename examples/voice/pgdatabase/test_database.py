...
class TestDatabase:
    ...
    async def test_open_connects_and_acquire_reuses_the_pool(self) -> None:
        db = database.Database(database.DatabaseRequest(os.environ["CALLS_STORAGE"]))
        await db.open()
    ...
    async def test_a_cancelled_query_releases_its_connection(self) -> None:
        db = database.Database(database.DatabaseRequest(os.environ["CALLS_STORAGE"]), min_size=1, max_size=1)
        await db.open()
...
class TestDatabases:
    ...
    async def test_open_and_close_reach_every_database(self) -> None:
        request = database.DatabaseRequest(os.environ["CALLS_STORAGE"])
