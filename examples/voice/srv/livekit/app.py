...
class LivekitApp(ts.Host):
    ...
    async def accept_job(self, job_request: livekit_agents.JobRequest) -> None:
        await self._livekit_handler.handle_accept_job_request(
            protocol.VoiceAcceptJobRequest(job_request=job_request)
        )

    async def start_job(self, job_context: livekit_agents.JobContext) -> None:
        await self._livekit_handler.handle_start_job_request(
            protocol.VoiceStartJobRequest(job_context=job_context)
        )
