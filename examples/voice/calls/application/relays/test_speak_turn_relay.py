...
def speak_turn_response(
    ...
    return relays.SpeakTurnResponse(call_id=call_id, text=text, person_names=(person_name,))
...
class TestSpeakTurnResponseSnapshot:
    ...
    def test_a_response_is_its_call_id_text_and_person_names(self) -> None:
        ...
        assert raw == b'{"call_id": "c1", "text": "nice to meet you, Ada", "person_names": ["Ada"]}'

    def test_a_response_comes_back_equal(self) -> None:
        speak_turn_response_snapshot = relays.SpeakTurnResponseSnapshot()

        assert speak_turn_response_snapshot.deserialize(
            speak_turn_response_snapshot.serialize(speak_turn_response())
        ) == speak_turn_response()

    def test_a_response_with_no_name_given_comes_back_with_none(self) -> None:
        speak_turn_response_snapshot = relays.SpeakTurnResponseSnapshot()
        no_name = relays.SpeakTurnResponse(call_id="c1", text="sorry, what was that?", person_names=())

        returned = speak_turn_response_snapshot.deserialize(speak_turn_response_snapshot.serialize(no_name))

        assert returned.person_names == ()

    def test_a_response_of_the_wrong_shape_is_refused_before_the_constructor(self) -> None:
        for raw in (
            b"{}",
            b'{"call_id": "c1", "text": "hi"}',
            b'{"call_id": "c1", "text": "hi", "person_names": [1]}',
            b'["c1"]',
        ):
            with pytest.raises(errors.DomainError):
