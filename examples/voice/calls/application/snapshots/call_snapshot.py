        ...
        return json.dumps(
            {
                ...
                "turns": [  # tesser:debt TB082
                    {"speaker": str(turn.speaker), "text": str(turn.utterance)} for turn in call.conversation.turns
                ],
                "step": str(call.step),
            }
        ).encode()
    ...
    def deserialize(self, buf: bytes) -> domain.Call:
        ...
        if not (
            ...
            and all(  # tesser:debt TB082
                isinstance(turn, dict) and isinstance(turn.get("speaker"), str) and isinstance(turn.get("text"), str)
        ...
        return domain.Call(
            domain.CallSpec(
                ...
                turns=tuple(  # tesser:debt TB082
                    domain.TurnSpec(speaker=turn["speaker"], text=turn["text"]) for turn in snapshot["turns"]
                ),
                step=snapshot["step"],
            )
        )
