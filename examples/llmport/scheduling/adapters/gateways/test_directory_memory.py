...
def test_the_directory_offers_the_slots_it_was_opened_with() -> None:
    ...
    list_available_slots_response = memory_slot_directory.list_available_slots(
        ports.ListAvailableSlotsRequest()
    )

    assert list_available_slots_response.slots == ("mon-9am", "tue-2pm")
...
def test_an_empty_directory_offers_nothing() -> None:
    ...
    assert list_available_slots_response.slots == ()


def test_reserving_an_offered_slot_takes_it_for_the_caller() -> None:
    memory_slot_directory = gateways.MemorySlotDirectory(("mon-9am", "tue-2pm"))

    reserve_slot_response = memory_slot_directory.reserve_slot(
        ports.ReserveSlotRequest(slot="mon-9am", name="Ada Lovelace")
    )

    assert reserve_slot_response.outcome is ports.ReserveSlotOutcome.RESERVED
...
def test_a_reserved_slot_is_no_longer_offered() -> None:
    memory_slot_directory = gateways.MemorySlotDirectory(("mon-9am", "tue-2pm"))
    memory_slot_directory.reserve_slot(ports.ReserveSlotRequest(slot="mon-9am", name="Ada"))

    list_available_slots_response = memory_slot_directory.list_available_slots(
        ports.ListAvailableSlotsRequest()
    )

    assert list_available_slots_response.slots == ("tue-2pm",)
...
def test_reserving_the_same_slot_twice_comes_back_taken_with_what_is_left() -> None:
    ...
    assert reserve_slot_response.outcome is ports.ReserveSlotOutcome.SLOT_TAKEN
...
def test_each_reservation_records_the_caller_who_made_it() -> None:
    ...
    memory_slot_directory.reserve_slot(ports.ReserveSlotRequest(slot="tue-2pm", name="Grace"))
    memory_slot_directory.reserve_slot(ports.ReserveSlotRequest(slot="mon-9am", name="Ada"))
