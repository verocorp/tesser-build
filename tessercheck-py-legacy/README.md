...
| Code | Rule |
|---|---|
| **TB019** | a domain object's public method returns a domain object ([`skills/tesser-build/domain-return.md`](../skills/tesser-build/domain-return.md)): a subclass of `ts.ValueObject` / `ts.Entity` / `ts.AggregateRoot` may not hand back a primitive, an enum, a foreign value (`Decimal`/`datetime`) or a DTO, and the six rich comparison dunders are in scope — CPython does not fix their return type, so a `-> bool` there is the same leak as a `before()` returning one; the base owns equality. Two exits: the dunders CPython DOES fix (measured — `__hash__`/`__str__`/`__bool__`/`__len__`/… all raise `TypeError` otherwise), and a `-> None` transition. Containers are read through to their payload; private and unannotated methods are not judged (`mypy --strict` already requires the annotation) |
...
| **TB033** | a builtin name **bound and then called in the same scope** — `def f(id: str): return id(x)` raises `TypeError: 'str' object is not callable`, and `len = 0` then `len(name)` does the same. Order carries no signal: Python scopes per *function*, so any assignment makes the name local for the whole body and a call placed earlier raises `UnboundLocalError` instead. This targets the **collision, not the name** — a spec field or a parameter merely called `id` is fine, because the builtin stays reachable everywhere a human writes code. That is the difference from ruff's `A001`/`A002`, which ban the name and on this repo flag a `BaseHTTPRequestHandler.log_message(self, format, *args)` override whose signature the stdlib fixes. Global scope; the escape hatch is `# tessercheck:ignore`, whose one expected use is a callable binding legitimately called (`def apply(filter, xs): return filter(xs)`) |
...
exercise domain objects; TB004, TB020, TB030 and TB033 fire everywhere. TB032 scopes
