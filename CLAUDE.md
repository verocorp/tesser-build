...
the Go `comments` analyzer), the annotations norm (`TB021` — an annotation is
...
- **`scripts/verify` covers more than the tree you are editing.** The
  ...
  `python-app`, `serdepy` — the other three `app` rows are `layout`,
  `tesser-py`, and `tessercheck-py`), so a
