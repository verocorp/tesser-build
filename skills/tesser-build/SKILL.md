...
| Writing a workflow on a durable-execution engine (Restate, Temporal) — an orchestrator, an action / activity, or the engine's callback | Read `python.md#orchestrators-actions-relays` — a relay (`ts.Relay`, `application/relays/`) is the protocol whose far side is this same context across the engine, declared with the messages it speaks and a snapshot for each; an orchestrator (`ts.Orchestrator`, `application/orchestrators/`) depends on relays and action ports and is reached through its application client, beside which a `ts.Workflow` protocol (generic in the engine's context) yields that client per invocation; a class of actions (`ts.Actions`) has one port and one call per method and never holds a relay; a runner (`ts.Runner`, `adapters/runners/`) implements a relay, reaching its far side by literal service and handler name, or a workflow, building the orchestrator over that invocation's runners; a runtime (`ts.Runtime`, `adapters/runtimes/`) registers the engine's handlers, holds application clients and workflows, constructs nothing else, and invokes no relay itself; no adapters kind package imports another |
