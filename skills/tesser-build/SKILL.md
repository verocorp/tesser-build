...
| A service needs to branch or loop on what the domain did (`while`, `match`, `if` on a domain call; "how do I get a bool out?") | Read `domain-return.md` rule 6 and `python.md#outcomes` — the transition returns a `ts.Outcome` (a closed set, a value object, never stored), the service matches it exhaustively with `assert_never`; never a bool, never a status accessor, never `== Outcome.X` |
