...
| Writing a workflow on a durable-execution engine (Restate, Temporal) — an orchestrator, an action / activity, or the engine's callback | Read `python.md#orchestrators-actions-jobs` — an orchestrator (`ts.Orchestrator`, `application/orchestrators/`) is built per invocation by a job and depends on action ports only; a class of actions (`ts.Actions`) has one port and one call per method; a job (`ts.Job`, `adapters/jobs/`) calls an application client (`application/client/`) or constructs the orchestrator; messages are declared once, on the port |
