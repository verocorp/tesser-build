...
| Writing a workflow on a durable-execution engine (Restate, Temporal) — an orchestrator, an action / activity, or the engine's callback | Read `python.md#orchestrators-actions-jobs` — an orchestrator (`ts.Orchestrator`, `application/orchestrators/`) is built per invocation by a job with that invocation's `ts.JobContext`, depends on that one job context plus action ports, and threads the job context as the leading argument of every action-port call; a class of actions (`ts.Actions`) has one port and one call per method; a job (`ts.Job`, `adapters/jobs/`) calls an application client (`application/client/`) or constructs the orchestrator; messages are declared once, on the port |
...
| Importing across packages, or asking what a package exposes | Read `python.md#building-domain-code-in-python` — outside a role package you import the package (`import alpha.domain as domain`), aliased to its last segment; the role `__init__` is the export list (`from x import Y as Y`); inside a role package no module imports a module beside it (merge them; only the sibling test may) and none imports its own package; no package or shell module carries a class of its own name |
...
| Naming a local, a parameter, or the field an `__init__` sets | Read `python.md#building-domain-code-in-python` — the name is the type in snake_case (`add_request: client.AddRequest`, `widget = domain.Widget(spec)`, `self._widget_repository = widget_repository`); a spec or DTO `__init__` parameter is a field name and is exempt, and so are two values of one class in one function |
