...
- **TB016** (the value-object-primitives check) flags rule 5's internal
  ...
  one primitive for a leaf, one spec otherwise, never value objects). A
  compound holds child
...
- Python mechanics: `python.md#value-objects` (canonical exits, child VOs),
  ...
  `examples/python-app/campaign/domain/campaign.py`; the walk-plus-DTO pair
  verified in `examples/serdepy/parcel/tests/test_record_construction.py`
  and `examples/serdepy/parcel/application/ports/parcel_wire.py`.
