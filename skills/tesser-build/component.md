...
whatever must be closed. The app-level composition root (`app.md`)
...
- The **composition root** (`app.md`) — app-wide assembly: calls every
...
3. **The context's `Config` lives in its wiring, never on the public interface.**
   `component/config` is spec-shaped: a frozen struct, primitive leaves, no
   methods, no constructor logic. The app `Config` nests the per-context ones
   (`app.md#app-config`); the public `Client` package never mentions
...
`examples/python-app/campaign/component/` (full case, with an injected
cross-context port), `examples/python-app/reports/component/` (minimal case: no
...
(`examples/python-app/campaign/component/test_component.py`,
`examples/python-app/linkpolicy/component/test_component.py`): the `"memory"` coordinate
...
- **The coordinate builds the implementation it names:** constructing the
  ...
  impl: `examples/python-app/campaign/component/test_component.py`).
...
- **The component closes what it built:** teardown is the component's own
  ...
  `examples/python-app/app/test_app.py` locks that the app closes each
  component it built).
