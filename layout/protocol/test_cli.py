...
def test_an_argument_at_its_index_comes_back() -> None:
    request = cli.CliRequest(args=("/repo", "--verbose"))
...
def test_a_missing_argument_names_itself_and_carries_the_usage() -> None:
    with pytest.raises(cli.UsageError) as raised:
        cli.CliRequest(args=()).arg(0, "repo-root", "usage: prog <repo-root>")
...
def test_an_argument_past_the_end_is_a_missing_argument() -> None:
    with pytest.raises(cli.UsageError):
        cli.CliRequest(args=("/repo",)).arg(1, "second", "usage: prog <repo-root>")


def test_the_exact_argument_count_passes_the_extra_argument_gate() -> None:
    request = cli.CliRequest(args=("/repo",))
...
def test_a_response_carries_the_exit_code_and_both_streams() -> None:
    response = cli.CliResponse(2, stdout="", stderr="boom")
...
def test_two_responses_with_the_same_fields_are_equal() -> None:
    assert cli.CliResponse.ok("done") == cli.CliResponse(0, stdout="done", stderr="")
    assert cli.CliResponse.ok("done") != cli.CliResponse(1, stdout="done", stderr="")


def test_a_request_holds_its_arguments_as_given() -> None:
    assert cli.CliRequest(args=("a", "b")).args == ("a", "b")
    assert cli.CliRequest(args=("a", "b")) == cli.CliRequest(args=("a", "b"))
