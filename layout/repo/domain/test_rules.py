...
def test_a_consistent_repo_has_no_problems() -> None:
    assert domain.Repo(_spec()).health() is domain.Health.CLEAN


def test_an_unknown_kind_is_a_problem() -> None:
    rows = _spec().manifest[1][:5] + (("libby", "python-library"), ("scripts", "ungated"))
    repo = domain.Repo(_spec(manifest=("read", rows, "")))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.UNKNOWN_KIND, "libby", "python-library"))

    assert presence is domain.Presence.AMONG


def test_a_top_level_dir_without_a_row_is_a_problem() -> None:
    top = _spec().top + (("utils", "directory"),)
    repo = domain.Repo(_spec(top=top))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.UNREGISTERED_TOP_LEVEL, "utils"))

    assert presence is domain.Presence.ONLY
...
def test_a_row_without_a_dir_is_a_problem() -> None:
    ...
    presence = repo.presence(domain.ProblemSpec(domain.Rule.ROW_WITHOUT_DIRECTORY, "ghost"))

    assert presence is domain.Presence.ONLY


def test_an_examples_dir_without_a_row_is_a_problem() -> None:
    repo = domain.Repo(_spec(examples=(("demo", "directory"), ("newthing", "directory"))))

    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.UNREGISTERED_EXAMPLE, "examples/newthing")
    )
...
def test_an_examples_row_without_a_dir_is_a_problem() -> None:
    repo = domain.Repo(_spec(examples=()))

    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.ROW_WITHOUT_DIRECTORY, "examples/demo")
    )
...
def test_a_symlinked_top_level_dir_is_a_problem() -> None:
    ...
    presence = repo.presence(domain.ProblemSpec(domain.Rule.SYMLINKED_DIRECTORY, "appone"))
...
def test_an_app_row_without_a_verify_arm_is_a_problem() -> None:
    ...
    repo = domain.Repo(_spec(verify=verify))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.NO_VERIFY_ARM, "libby"))
...
def test_an_app_row_without_a_ci_job_is_a_problem() -> None:
    ...
    repo = domain.Repo(_spec(workflow=workflow))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.NO_CI_JOB, "libby"))
...
def test_a_tessercheck_arm_without_a_declaration_is_a_problem() -> None:
    repo = domain.Repo(_spec(declarations=()))

    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.DECLARATION_MISSING, "appone/.tesser-root")
    )
...
def test_a_declaration_without_a_tessercheck_arm_is_a_problem() -> None:
    ...
    repo = domain.Repo(_spec(declarations=declarations))

    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.DECLARATION_UNCHECKED, "libby/.tesser-root")
    )
...
def test_a_wrong_declaration_first_line_is_a_problem() -> None:
    ...
    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.DECLARATION_NOT_APP, "appone/.tesser-root")
    )
...
def test_an_unreadable_declaration_is_a_problem() -> None:
    ...
    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.DECLARATION_UNAVAILABLE, "appone/.tesser-root", "unreadable")
    )
...
def test_a_skip_line_in_a_declaration_is_recognized() -> None:
    ...
    assert domain.Repo(_spec(declarations=declarations)).health() is domain.Health.CLEAN


def test_a_requirements_file_outside_an_app_row_is_a_problem() -> None:
    repo = domain.Repo(_spec(requirements=("appone", "libby", "docs")))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.REQUIREMENTS_OUTSIDE_APP, "docs"))

    assert presence is domain.Presence.ONLY


def test_a_deep_requirements_file_is_caught() -> None:
    repo = domain.Repo(_spec(requirements=("appone", "libby", "docs/buried/tree")))

    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.REQUIREMENTS_OUTSIDE_APP, "docs/buried/tree")
    )
...
def test_a_demoted_app_row_is_caught_by_the_requirements_rule() -> None:
    ...
    presence = repo.presence(domain.ProblemSpec(domain.Rule.REQUIREMENTS_OUTSIDE_APP, "libby"))
...
def test_shared_app_directory_names_are_a_problem() -> None:
    ...
    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.SHARED_GATE_NAME, "libby", "examples/libby")
    )
...
def test_a_malformed_manifest_is_the_only_problem() -> None:
    repo = domain.Repo(_spec(manifest=("malformed", (), "Expecting value: line 1")))

    presence = repo.presence(
        domain.ProblemSpec(
            domain.Rule.MANIFEST_MALFORMED, "manifest.json", "Expecting value: line 1"
        )
    )
...
def test_a_misshapen_manifest_is_the_only_problem() -> None:
    repo = domain.Repo(_spec(manifest=("misshapen", (), "")))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.MANIFEST_MISSHAPEN, "manifest.json"))

    assert presence is domain.Presence.ONLY


def test_a_missing_manifest_is_the_only_problem() -> None:
    repo = domain.Repo(_spec(manifest=("missing", (), "")))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.MANIFEST_MISSING, "manifest.json"))
...
def test_an_absent_problem_is_absent() -> None:
    ...
    assert presence is domain.Presence.ABSENT


def test_trees_lists_app_rows_in_manifest_order() -> None:
    repo = domain.Repo(_spec())
...
def test_a_missing_verify_file_is_a_problem_and_cascades() -> None:
    repo = domain.Repo(_spec(verify=("missing", "")))

    unavailable = repo.presence(
        domain.ProblemSpec(domain.Rule.VERIFY_UNAVAILABLE, "scripts/verify", "missing")
    )
    cascaded = repo.presence(domain.ProblemSpec(domain.Rule.NO_VERIFY_ARM, "appone"))

    assert unavailable is domain.Presence.AMONG
    assert cascaded is domain.Presence.AMONG


def test_an_unreadable_workflow_file_is_a_problem() -> None:
    repo = domain.Repo(_spec(workflow=("unreadable", "")))

    presence = repo.presence(
        domain.ProblemSpec(
            domain.Rule.WORKFLOW_UNAVAILABLE, ".github/workflows/test.yml", "unreadable"
        )
    )
...
def test_an_unreadable_manifest_is_the_only_problem() -> None:
    repo = domain.Repo(_spec(manifest=("unreadable", (), "")))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.MANIFEST_UNREADABLE, "manifest.json"))

    assert presence is domain.Presence.ONLY


def test_trees_and_counts_degrade_when_the_manifest_cannot_be_read() -> None:
    repo = domain.Repo(_spec(manifest=("malformed", (), "boom")))
    assert repo.trees() == ()
    assert tuple(str(count) for count in repo.counts()) == ("0", "0")


def test_a_symlinked_examples_dir_reports_with_its_prefix() -> None:
    repo = domain.Repo(_spec(examples=(("demo", "symlink"),)))

    presence = repo.presence(domain.ProblemSpec(domain.Rule.SYMLINKED_DIRECTORY, "examples/demo"))
...
def test_text_equality() -> None:
    assert domain.Text("a") == domain.Text("a")
    assert domain.Text("a") != domain.Text("b")


def test_problem_constructs_from_spec() -> None:
    problem_spec = domain.ProblemSpec(domain.Rule.UNKNOWN_KIND, "libby", "python-library")

    problem = domain.Problem(problem_spec)

    assert str(problem.subject()) == problem_spec.subject
    assert str(problem.note()) == problem_spec.note
...
def test_problem_equality() -> None:
    assert domain.Problem(domain.ProblemSpec(domain.Rule.NO_CI_JOB, "a")) == domain.Problem(
        domain.ProblemSpec(domain.Rule.NO_CI_JOB, "a")
    )
    assert domain.Problem(domain.ProblemSpec(domain.Rule.NO_CI_JOB, "a")) != domain.Problem(
        domain.ProblemSpec(domain.Rule.NO_VERIFY_ARM, "a")
    )
    assert domain.Problem(domain.ProblemSpec(domain.Rule.NO_CI_JOB, "a")) != domain.Problem(
        domain.ProblemSpec(domain.Rule.NO_CI_JOB, "b")
    )
...
def test_an_empty_manifest_key_is_a_row_without_a_directory_not_a_crash() -> None:
    ...
    presence = repo.presence(domain.ProblemSpec(domain.Rule.ROW_WITHOUT_DIRECTORY, ""))
...
@pytest.mark.parametrize(
    ...
    [
        (domain.Rule.MANIFEST_MISSING, "manifest.json", "", "manifest.json is missing"),
        (domain.Rule.MANIFEST_UNREADABLE, "manifest.json", "", "manifest.json is unreadable"),
        (
            domain.Rule.MANIFEST_MALFORMED,
            "manifest.json",
            "Expecting value",
            "manifest.json is unreadable: Expecting value",
        ),
        (
            domain.Rule.MANIFEST_MISSHAPEN,
            "manifest.json",
            "",
            "manifest.json is not a flat object of directory-path to kind strings",
        ),
        (
            domain.Rule.UNKNOWN_KIND,
            "libby",
            "python-library",
            "manifest.json row 'libby' declares unknown kind 'python-library'; "
            "the kinds are 'app' and 'ungated'",
        ...
        (
            domain.Rule.UNREGISTERED_TOP_LEVEL,
            "utils",
            "",
        ...
        (
            domain.Rule.UNREGISTERED_EXAMPLE,
            "examples/newthing",
            "",
            "examples/newthing has no manifest.json row",
        ),
        (
            domain.Rule.ROW_WITHOUT_DIRECTORY,
            "examples/demo",
            "",
            "manifest.json row 'examples/demo' names no directory on disk",
        ),
        (
            domain.Rule.SYMLINKED_DIRECTORY,
            "appone",
            "",
            "appone is a symlinked directory; a declared directory is a real one",
        ),
        (domain.Rule.VERIFY_UNAVAILABLE, "scripts/verify", "missing", "scripts/verify is missing"),
        (
            domain.Rule.WORKFLOW_UNAVAILABLE,
            ".github/workflows/test.yml",
            "unreadable",
            ".github/workflows/test.yml is unreadable",
        ),
        (
            domain.Rule.SHARED_GATE_NAME,
            "libby",
            "examples/libby",
        ...
        (
            domain.Rule.REQUIRES_PYTHON_UNDECLARED,
        ...
        (
            domain.Rule.TARGET_VERSION_UNDECLARED,
            "libby/ruff.toml",
            "",
        ...
        (
            domain.Rule.FLOOR_FILE_UNAVAILABLE,
            "libby/pyproject.toml",
            "malformed",
            "libby/pyproject.toml is malformed",
        ),
        (
            domain.Rule.REQUIRES_PYTHON_BELOW_FLOOR,
            "libby/pyproject.toml",
            ">=3.11",
        ...
        (
            domain.Rule.TARGET_VERSION_BELOW_FLOOR,
            "libby/ruff.toml",
            "py311",
        ...
        (
            domain.Rule.WORKFLOW_PIN_BELOW_FLOOR,
            ".github/workflows/test.yml",
            "3.11",
...
def test_every_rule_renders_its_sentence(
    rule: domain.Rule, subject: str, note: str, sentence: str
) -> None:
    problem = domain.Problem(domain.ProblemSpec(rule, subject, note))

    assert str(problem.text()) == sentence


@pytest.mark.parametrize("rule", list(domain.Rule))
def test_every_rule_renders_a_sentence(rule: domain.Rule) -> None:
    problem = domain.Problem(domain.ProblemSpec(rule, "libby/thing", "note"))
...
def test_a_trailing_slash_app_row_is_a_problem_and_trees_survives_it() -> None:
    ...
    presence = repo.presence(domain.ProblemSpec(domain.Rule.NO_VERIFY_ARM, "ghost/"))
...
def test_a_requires_python_below_the_floor_is_a_problem() -> None:
    ...
    repo = domain.Repo(_spec(floors=floors))

    presence = repo.presence(
        domain.ProblemSpec(
            domain.Rule.REQUIRES_PYTHON_BELOW_FLOOR, "libby/pyproject.toml", ">=3.11"
        )
    )
...
def test_a_whitespaced_requires_python_at_the_floor_is_clean() -> None:
    ...
    assert domain.Repo(_spec(floors=floors)).health() is domain.Health.CLEAN
...
def test_a_distribution_without_a_requires_python_is_a_problem() -> None:
    ...
    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.REQUIRES_PYTHON_UNDECLARED, "libby/pyproject.toml")
    )
...
def test_a_distribution_without_a_target_version_is_a_problem() -> None:
    ...
    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.TARGET_VERSION_UNDECLARED, "libby/ruff.toml")
    )
...
def test_a_target_version_below_the_floor_is_a_problem() -> None:
    ...
    presence = repo.presence(
        domain.ProblemSpec(domain.Rule.TARGET_VERSION_BELOW_FLOOR, "libby/ruff.toml", "py311")
    )
...
def test_an_unparsable_config_file_is_a_problem() -> None:
    ...
    malformed = repo.presence(
        domain.ProblemSpec(domain.Rule.FLOOR_FILE_UNAVAILABLE, "libby/pyproject.toml", "malformed")
    )
    unreadable = repo.presence(
        domain.ProblemSpec(domain.Rule.FLOOR_FILE_UNAVAILABLE, "libby/ruff.toml", "unreadable")
    )

    assert malformed is domain.Presence.AMONG
    assert unreadable is domain.Presence.AMONG
...
def test_a_workflow_pin_below_the_floor_is_a_problem() -> None:
    ...
    presence = repo.presence(
        domain.ProblemSpec(
            domain.Rule.WORKFLOW_PIN_BELOW_FLOOR, ".github/workflows/test.yml", "3.11"
        )
    )
...
def test_a_matrix_entry_below_the_floor_is_a_problem() -> None:
    ...
    at_floor = repo.presence(
        domain.ProblemSpec(
            domain.Rule.WORKFLOW_PIN_BELOW_FLOOR, ".github/workflows/test.yml", "3.12"
        )
    )

    assert below is domain.Presence.ONLY
    assert at_floor is domain.Presence.ABSENT
...
def test_a_matrix_at_and_above_the_floor_is_clean() -> None:
    ...
    assert domain.Repo(_spec(workflow=workflow)).health() is domain.Health.CLEAN
